...
- **`gha-cf-tf-plan`** — read-only (`roles/viewer` + `roles/iam.securityReviewer` so plan can refresh resources and their IAM policies, plus object read/lock on the state bucket). Used by the `terraform plan` workflow on pull requests. It cannot write resources or read secret values, so a malicious PR cannot escalate through it.
